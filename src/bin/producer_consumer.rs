...
struct Args {
    #[arg(long, default_value = "/Users/lucaslima/Library/Application Support/Bitcoin/signet_new/blocks")]
    blocks_dir: PathBuf,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let base_seed: u64 = match args.seed {
        Some(s) => s,
        None => ChaCha8Rng::from_entropy().get_seed().iter().fold(0u64, |acc, &b| acc.wrapping_mul(257).wrapping_add(b as u64)),
    };


    println!(
        "blocks_dir={} | epoch_size={} | c={} | delta={} | trials={} |",
    ...
    for trial in 0..args.trials {
        ...
        writeln!(csv, "epoch_idx,k,n_collected,decode_success,c,delta,trial")?;

        let mut epochs_processed = 0usize;
        ...
        let mut epoch_visitor = |batch: EpochBatch| -> Result<ControlFlow<()>, ChainError> {
            ...
            let dist = RobustSoliton::new(k, args.c, args.delta);
            ...
            if !already_succeeded {
                println!(
                    "  Trial {:4} | Epoch {:3}: k={} | not decoded {} droplets",
                    trial + 1, batch.index, k, max_droplets
                );
            }
